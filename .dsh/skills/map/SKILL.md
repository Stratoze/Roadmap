...
- **Self-report routes; it is never evidence.** "I know that" means test it;
  "no idea" is data.
- **Work first.** No pre-work forms or gates - open the rung when the learner
  is ready. The session log is drafted at close, from evidence.
...
7. **Write results** in `## Map`: `provisional (<date>)` notes per strand
   ...
   the same way in `_system/learning/maps/overview.md` (strand = unit).
