...
> Core protocols. The *method*, not the content.
...
- **Metacognition:** `What assumption was wrong?`
...
## Memory / SRS Strategy
- **The vault learning system owns spaced repetition for concepts & procedures** (transparent ladder in `_system/learning/curriculum/*.md`; `python3 scripts/review.py due|frontier|schedule|next|usage|selftest`; reviews via the `review` skill, cold recall first). `due` lists what is stale to review and ignores prerequisites; `frontier` lists what is ready to teach. Never merge the two: reviewing what you learned and teaching what comes next are different jobs.
...
- **Landmine** = a failure mode worth reading before starting, because finding it in the wrong order costs days.
...
- **Yellow (reconstruct before using):** skeletons, register checklists, derivation hints, circuit suggestions.
